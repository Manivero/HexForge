...
| `hexforge-ops` — 49+ операции | ✅ | `cargo test -p hexforge-ops` — 220 тестов (см. ниже); `clippy -D warnings` 0 |
...
| Hashing | ✅ | blake3 **streamable**, blake2b/s **streamable**, crc32 **streamable** (Constant, 1 new chunked test), md5/sha1/sha256/sha512/sha3_256 **streamable** (Constant, 2 new chunked tests), ssdeep, hmac (5 tests), pbkdf2 (sha1/sha256/sha512, 3 tests, RFC6070) — known vectors |
...
Все изменения перепроверены: `cargo test --workspace` — 306 зелёных (Rust: core 9, ops 220, stream 7, engine 34, tauri 22, cli 4, plugin-host 10) + 42 FE (fuzz, bytes, graph, i18n, ipc) — `cargo clippy -D warnings` 0, `tsc --noEmit` 0, `eslint` 0, `vite build` 255kB, `npm run test:fe` 42.
