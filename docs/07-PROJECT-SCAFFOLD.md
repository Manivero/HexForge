...
| `hexforge-ops` — 50+ операции | ✅ | `cargo test -p hexforge-ops` — 225 тестов (см. ниже); `clippy -D warnings` 0 |
...
| Hashing | ✅ | blake3 **streamable**, blake2b/s **streamable**, crc32 **streamable** (Constant, 1 new chunked test), md5/sha1/sha256/sha512/sha3_256 **streamable** (Constant, 2 new chunked tests), ssdeep, hmac (5 tests), pbkdf2 (sha1/sha256/sha512, 4 tests + DoS cap 1M, RFC6070), scrypt (RFC 7914, 4 tests, log_n/r/p caps) — known vectors |
...
Все изменения перепроверены: `cargo test --workspace` — 311 зелёных (Rust: core 9, ops 225, stream 7, engine 34, tauri 22, cli 4, plugin-host 10) + 42 FE (fuzz, bytes, graph, i18n, ipc) — `cargo clippy -D warnings` 0, `tsc --noEmit` 0, `eslint` 0, `vite build` 255kB, `npm run test:fe` 42.
