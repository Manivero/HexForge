...
| `hexforge-ops` — 46+ операции | ✅ | `cargo test -p hexforge-ops` — 206 теста (см. ниже); `clippy -D warnings` 0 |
...
Все изменения перепроверены: `cargo test --workspace` — 292 зелёных (Rust: core 9, ops 206, stream 7, engine 34, tauri 22, cli 4, plugin-host 10) + 40 FE (fuzz, bytes, graph, i18n) — `cargo clippy -D warnings` 0, `tsc --noEmit` 0, `eslint` 0, `vite build` 253kB, `npm run test:fe` 40.
