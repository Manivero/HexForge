    ...
    fn apply<'a>(
        ...
        if data.len() < 24 {
            return Err(TransformError::InvalidInput {
                reason: "too short for PCAP global header".into(),
            });
        }
        ...
        let le = match magic_be {
            ...
            _ => {
                return Err(TransformError::InvalidInput {
                    reason: format!("invalid PCAP magic 0x{magic_be:08x}"),
                })
            }
        };
                        ...
                        if (proto == 6 || proto == 17) && pkt_data.len() >= ip_start + ihl + 4 {
                            ...
                            if pkt_data.len() >= tcp_start + 4 {
                                let src_port = u16::from_be_bytes([
                                    pkt_data[tcp_start],
                                    pkt_data[tcp_start + 1],
                                ]);
                                let dst_port = u16::from_be_bytes([
                                    pkt_data[tcp_start + 2],
                                    pkt_data[tcp_start + 3],
                                ]);
...
mod tests {
    ...
    fn make_pcap_with_ip() -> Vec<u8> {
        ...
        pkt.extend_from_slice(&[
            0x45, 0x00, 0x00, 0x28, 0x00, 0x00, 0x40, 0x00, 0x40, 0x06, 0x00, 0x00, 192, 168, 1, 1,
            10, 0, 0, 1,
        ]); // ip
        pkt.extend_from_slice(&[
            0x04, 0xd2, 0x00, 0x50, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x50, 0x02,
    ...
    fn rejects_invalid_magic() {
        let ctx = NullExecutionContext;
        let err = PcapParse
            .apply(Cow::Borrowed(b"bad"), &serde_json::json!({}), &ctx)
            .unwrap_err();
