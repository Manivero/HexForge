...
impl Transform for DnsParse {
    ...
    fn apply<'a>(&self, input: ByteView<'a>, _params: &serde_json::Value, _ctx: &dyn ExecutionContext) -> Result<ByteView<'a>, TransformError> {
        ...
        if data.len() < 12 {
            return Err(TransformError::InvalidInput { reason: "too short for DNS header (need 12)".into() });
        }
...
fn parse_name(data: &[u8], mut off: usize) -> Result<(String, usize), TransformError> {
    ...
    while off < data.len() {
        if steps > 64 {
            return Err(TransformError::InvalidInput { reason: "DNS name too many jumps".into() });
        }
        if !visited_offsets.insert(off) {
            return Err(TransformError::InvalidInput { reason: "DNS name pointer loop".into() });
        }
        steps += 1;
        ...
        if (len & 0xC0) == 0xC0 {
            ...
            if off + 1 >= data.len() {
                return Err(TransformError::InvalidInput { reason: "truncated DNS pointer".into() });
            }
            let ptr = ((len & 0x3F) << 8) | data[off + 1] as usize;
            if ptr >= data.len() {
                return Err(TransformError::InvalidInput { reason: format!("DNS pointer out of bounds: {ptr} >= {}", data.len()) });
            }
            if !jumped {
                next_off = off + 2;
            }
        ...
        if len > 63 {
            return Err(TransformError::InvalidInput { reason: format!("invalid DNS label length {len} (>63)") });
        }
        if off + 1 + len > data.len() {
            return Err(TransformError::InvalidInput { reason: "truncated DNS label".into() });
        }
...
mod tests {
    ...
    fn parses_query() {
        ...
        let data = make_query();
        let out = DnsParse.apply(Cow::Borrowed(&data), &serde_json::json!({}), &ctx).unwrap();
    ...
    fn rejects_short() {
        let ctx = NullExecutionContext;
        let err = DnsParse.apply(Cow::Borrowed(b"short"), &serde_json::json!({}), &ctx).unwrap_err();
    ...
    fn pointer_compression() {
        ...
        v.extend_from_slice(&1u16.to_be_bytes());
        v.extend_from_slice(&1u16.to_be_bytes());
        let out = DnsParse.apply(Cow::Borrowed(&v), &serde_json::json!({}), &ctx).unwrap();
    ...
    fn truncated_pointer_rejected() {
        ...
        let err = DnsParse.apply(Cow::Borrowed(&pkt), &serde_json::json!({}), &ctx).unwrap_err();
        assert!(matches!(err, TransformError::InvalidInput { .. }));
    }
    ...
    fn pointer_out_of_bounds_rejected() {
        ...
        let err = DnsParse.apply(Cow::Borrowed(&pkt), &serde_json::json!({}), &ctx).unwrap_err();
        assert!(matches!(err, TransformError::InvalidInput { .. }));
        let msg = match err { TransformError::InvalidInput { reason } => reason, _ => String::new() };
