...
impl Transform for ElfInfo {
    ...
    fn apply<'a>(&self, input: ByteView<'a>, _params: &serde_json::Value, _ctx: &dyn ExecutionContext) -> Result<ByteView<'a>, TransformError> {
        let obj = goblin::Object::parse(input.as_ref()).map_err(|e| TransformError::InvalidInput { reason: format!("ELF parse failed: {e}") })?;
        match obj {
            ...
            _ => Err(TransformError::InvalidInput { reason: "not an ELF file (magic 0x7F ELF not found)".into() }),
        }
    }
}
...
impl Transform for PeInfo {
    ...
    fn apply<'a>(&self, input: ByteView<'a>, _params: &serde_json::Value, _ctx: &dyn ExecutionContext) -> Result<ByteView<'a>, TransformError> {
        let obj = goblin::Object::parse(input.as_ref()).map_err(|e| TransformError::InvalidInput { reason: format!("PE parse failed: {e}") })?;
        match obj {
            goblin::Object::PE(pe) => {
                let out = serde_json::json!({
            ...
            _ => Err(TransformError::InvalidInput { reason: "not a PE file (MZ header not found)".into() }),
        }
    }
}
...
mod tests {
    ...
    fn elf_info_rejects_non_elf() {
        let ctx = NullExecutionContext;
        let err = ElfInfo.apply(Cow::Borrowed(b"MZ fake pe"), &json!({}), &ctx).unwrap_err();
    ...
    fn pe_info_rejects_non_pe() {
        let ctx = NullExecutionContext;
        let err = PeInfo.apply(Cow::Borrowed(b"\x7FELF fake elf"), &json!({}), &ctx).unwrap_err();
        assert!(matches!(err, TransformError::InvalidInput { .. }));
    }
