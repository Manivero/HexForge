...
fn parse_hex_param(
    ...
    let Some(v) = params.get(field) else {
        if required {
            return Err(TransformError::InvalidParameter {
                field: field.into(),
                reason: format!("hex parameter '{field}' is required"),
            });
        } else {
            return Ok(None);
        }
    };
    let s = v.as_str().ok_or_else(|| TransformError::InvalidParameter {
        field: field.into(),
        reason: format!("'{field}' must be a hex string"),
    })?;
    ...
    hex::decode(&clean)
        ...
        .map_err(|e| TransformError::InvalidParameter {
            field: field.into(),
            reason: format!("invalid hex for '{field}': {e}"),
        })
}
    ...
    K: cipher::BlockCipher + cipher::BlockDecrypt,
{
    ...
    let pt = EcbDecryptor::<K>::new_from_slice(key)
        ...
        .map_err(|e| TransformError::InvalidInput {
            reason: format!("AES ECB decrypt failed: {e}"),
        })?;
    Ok(pt.to_vec())
}
    ...
    K: cipher::BlockCipher + cipher::BlockDecrypt,
{
    ...
    let pt = CbcDecryptor::<K>::new_from_slices(key, iv)
        ...
        .decrypt_padded_mut::<Pkcs7>(&mut buf)
        .map_err(|e| TransformError::InvalidInput {
            reason: format!("AES CBC decrypt failed: {e}"),
        })?;
    Ok(pt.to_vec())
}
...
fn aes_encrypt_ecb(key: &[u8], data: &[u8]) -> Result<Vec<u8>, TransformError> {
    match key.len() {
        ...
        _ => Err(TransformError::InvalidParameter {
            field: "key".into(),
            reason: format!("AES key must be 16/24/32 bytes (got {})", key.len()),
        }),
    }
}
...
fn aes_encrypt_cbc(key: &[u8], iv: &[u8], data: &[u8]) -> Result<Vec<u8>, TransformError> {
    if iv.len() != 16 {
        return Err(TransformError::InvalidParameter {
            field: "iv".into(),
            reason: format!("AES CBC iv must be 16 bytes (got {})", iv.len()),
        });
    }
    match key.len() {
...
fn aes_crypt_ctr(key: &[u8], iv: &[u8], data: &[u8]) -> Result<Vec<u8>, TransformError> {
    if iv.len() != 16 {
        return Err(TransformError::InvalidParameter {
            field: "iv".into(),
            reason: format!("AES CTR iv must be 16 bytes (got {})", iv.len()),
        });
    }
    let mut buf = data.to_vec();
    match key.len() {
...
impl Transform for AesEncrypt {
    ...
    fn apply<'a>(
        ...
        let key = parse_hex_param(params, "key", true)?.ok_or_else(|| {
            TransformError::InvalidParameter {
                field: "key".into(),
                reason: "hex parameter 'key' is required".into(),
            }
        })?;
        ...
        let out = match mode {
            ...
            "cbc" => {
                let iv = parse_hex_param(params, "iv", true)?.ok_or_else(|| {
                    TransformError::InvalidParameter {
                        field: "iv".into(),
                        reason: "hex parameter 'iv' is required for cbc".into(),
                    }
                })?;
            ...
            "ctr" => {
                let iv = parse_hex_param(params, "iv", true)?.ok_or_else(|| {
                    TransformError::InvalidParameter {
                        field: "iv".into(),
                        reason: "hex parameter 'iv' is required for ctr".into(),
                    }
                })?;
            ...
            _ => {
                return Err(TransformError::InvalidParameter {
                    field: "mode".into(),
                    reason: "mode must be ecb|cbc|ctr".into(),
                })
            }
        };
        Ok(Cow::Owned(out))
...
mod tests {
    ...
    fn missing_iv_for_cbc_rejected() {
        ...
        let err = AesEncrypt
            .apply(
                Cow::Borrowed(b"x"),
                &serde_json::json!({"key": "00112233445566778899aabbccddeeff", "mode": "cbc"}),
                &ctx,
            )
            .unwrap_err();
        assert!(matches!(err, TransformError::InvalidParameter { .. }));
    ...
    fn ctr_missing_iv_rejected() {
        ...
        let err = AesEncrypt
            .apply(
                Cow::Borrowed(b"x"),
                &serde_json::json!({"key": "00112233445566778899aabbccddeeff", "mode": "ctr"}),
                &ctx,
            )
            .unwrap_err();
        assert!(matches!(err, TransformError::InvalidParameter { .. }));
