...
mod tests {
    ...
    fn brute_force_finds_key() {
        ...
        let out = XorBruteForce
            .apply(Cow::Borrowed(&ct), &serde_json::json!({}), &ctx)
            .unwrap();
        let s = String::from_utf8(out.into_owned()).unwrap();
        assert!(
            s.contains("2a: Hello"),
            "should contain key 2a -> Hello, got {s}"
        );
    }
    ...
    fn printable_filter() {
        ...
        let out = XorBruteForce
            .apply(
                Cow::Borrowed(&ct),
                &serde_json::json!({"printableOnly": true}),
                &ctx,
            )
            .unwrap();
        ...
        let s = String::from_utf8(out.into_owned()).unwrap();
        assert!(!s.is_empty());
        let out_all = XorBruteForce
            .apply(
                Cow::Borrowed(&ct),
                &serde_json::json!({"printableOnly": false}),
                &ctx,
            )
            .unwrap();
