...
impl Transform for ChaCha20Poly1305Encrypt {
    ...
    fn apply<'a>(
        ...
        if key.len() != 32 {
            return Err(TransformError::InvalidParameter { field: "key".into(), reason: format!("ChaCha20-Poly1305 key must be 32 bytes (got {})", key.len()) });
        }
        if nonce.len() != 12 {
            return Err(TransformError::InvalidParameter { field: "nonce".into(), reason: "ChaCha20-Poly1305 nonce must be 12 bytes (96 bits)".into() });
        }

        let cipher = ChaCha20Poly1305::new_from_slice(&key).map_err(|e| TransformError::InvalidParameter { field: "key".into(), reason: format!("ChaCha20-Poly1305 init failed: {e}") })?;
        let nonce = Nonce::from_slice(&nonce);
        let ciphertext = cipher.encrypt(nonce, chacha20poly1305::aead::Payload { msg: input.as_ref(), aad: &aad })
            .map_err(|e| TransformError::Internal(format!("ChaCha20-Poly1305 encrypt failed: {e}")))?;
...
impl Transform for ChaCha20Poly1305Decrypt {
    ...
    fn apply<'a>(
        ...
        let cipher = ChaCha20Poly1305::new_from_slice(&key).map_err(|e| TransformError::InvalidParameter { field: "key".into(), reason: format!("ChaCha20-Poly1305 init failed: {e}") })?;
        let nonce = Nonce::from_slice(&nonce);
        let plaintext = cipher.decrypt(nonce, chacha20poly1305::aead::Payload { msg: input.as_ref(), aad: &aad })
            .map_err(|e| TransformError::InvalidInput { reason: format!("ChaCha20-Poly1305 decrypt failed: {e}") })?;
...
mod tests {
    ...
    fn chacha20_poly1305_roundtrip() {
        ...
        let enc = ChaCha20Poly1305Encrypt.apply(Cow::Borrowed(pt), &params, &ctx).unwrap();
    ...
    fn rejects_invalid_key() {
        let ctx = NullExecutionContext;
        let err = ChaCha20Poly1305Encrypt.apply(Cow::Borrowed(b"x"), &serde_json::json!({"key": "0011", "nonce": "00".repeat(12)}), &ctx).unwrap_err();
    ...
    fn rejects_invalid_nonce() {
        let ctx = NullExecutionContext;
        let err = ChaCha20Poly1305Encrypt.apply(Cow::Borrowed(b"x"), &serde_json::json!({"key": "00".repeat(32), "nonce": "00"}), &ctx).unwrap_err();
        assert!(matches!(err, TransformError::InvalidParameter { .. }));
    }
    ...
    fn rejects_tampered_ciphertext() {
        ...
        let nonce = "00".repeat(12);
        let params = serde_json::json!({"key": key, "nonce": nonce});
        let enc = ChaCha20Poly1305Encrypt.apply(Cow::Owned(b"test".to_vec()), &params, &ctx).unwrap();
        // Tamper with ciphertext
        let len = enc.len();
        if len > 0 {
            let mut enc_mut = enc.into_owned();
            enc_mut[len - 1] ^= 0xFF;
            let err = ChaCha20Poly1305Decrypt.apply(Cow::Owned(enc_mut), &params, &ctx).unwrap_err();
