    ...
    fn apply<'a>(&self, input: ByteView<'a>, _params: &serde_json::Value, _ctx: &dyn ExecutionContext) -> Result<ByteView<'a>, TransformError> {
        ...
        loop {
            match reader.read_event() {
                Ok(Event::Eof) => break,
                Ok(e) => {
                    writer.write_event(e).map_err(|e| TransformError::InvalidInput { reason: format!("XML write error: {e}") })?;
                }
                Err(e) => {
                    return Err(TransformError::InvalidInput { reason: format!("not valid XML: {e}") });
                }
            }
        }
        let out = writer.into_inner().into_inner();
        if out.is_empty() {
            return Err(TransformError::InvalidInput { reason: "input is empty or not XML".into() });
        }
        Ok(Cow::Owned(out))
    }
}
...
mod tests {
    ...
    fn pretty_formats() {
        ...
        let out = XmlPretty.apply(Cow::Borrowed(input), &serde_json::json!({}), &ctx).unwrap();
        ...
        let out2 = XmlPretty.apply(Cow::Borrowed(s.as_bytes()), &serde_json::json!({}), &ctx).unwrap();
    ...
    fn rejects_invalid_xml() {
        ...
        let err = XmlPretty.apply(Cow::Borrowed(b"<root"), &serde_json::json!({}), &ctx).unwrap_err();
