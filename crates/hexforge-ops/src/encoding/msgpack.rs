...
impl Transform for MsgpackDecode {
    ...
    fn apply<'a>(&self, input: ByteView<'a>, _params: &serde_json::Value, _ctx: &dyn ExecutionContext) -> Result<ByteView<'a>, TransformError> {
        let v: serde_json::Value = rmp_serde::from_slice(input.as_ref()).map_err(|e| TransformError::InvalidInput { reason: format!("not valid MessagePack: {e}") })?;
...
mod tests {
    ...
    fn decode_roundtrip() {
        ...
        let out = MsgpackDecode.apply(Cow::Borrowed(&msgpack), &json!({}), &ctx).unwrap();
    ...
    fn rejects_invalid_msgpack() {
        ...
        let err = MsgpackDecode.apply(Cow::Borrowed(&[0xC1]), &json!({}), &ctx).unwrap_err();
