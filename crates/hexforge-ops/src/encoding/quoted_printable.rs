...
fn qp_decode(data: &[u8]) -> Result<Vec<u8>, TransformError> {
    ...
    while i < data.len() {
        if data[i] == b'=' {
            ...
            let hi = (data[i + 1] as char).to_digit(16).ok_or_else(|| TransformError::InvalidInput {
                reason: format!("invalid quoted-printable escape at byte {i}: '{}' is not hex", data[i + 1] as char),
            })? as u8;
            let lo = (data[i + 2] as char).to_digit(16).ok_or_else(|| TransformError::InvalidInput {
                reason: format!("invalid quoted-printable escape at byte {i}: '{}' is not hex", data[i + 2] as char),
            })? as u8;
            out.push(hi * 16 + lo);
            i += 3;
        } else {
...
mod tests {
    ...
    fn roundtrip() {
        ...
        for data in [b"Hello World" as &[u8], b"=", b"\x00\xFF", b"a=b c"] {
            let enc = QuotedPrintableEncode.apply(Cow::Borrowed(data), &serde_json::json!({}), &ctx).unwrap();
            let dec = QuotedPrintableDecode.apply(enc, &serde_json::json!({}), &ctx).unwrap();
    ...
    fn encode_equals_and_nonprintable() {
        let ctx = NullExecutionContext;
        let out = QuotedPrintableEncode.apply(Cow::Borrowed(b"a=b"), &serde_json::json!({}), &ctx).unwrap();
    ...
    fn soft_break_ignored_on_decode() {
        let ctx = NullExecutionContext;
        let dec = QuotedPrintableDecode.apply(Cow::Borrowed(b"Hello=\r\nWorld"), &serde_json::json!({}), &ctx).unwrap();
    ...
    fn decode_rejects_invalid_escape() {
        let ctx = NullExecutionContext;
        let err = QuotedPrintableDecode.apply(Cow::Borrowed(b"=ZZ"), &serde_json::json!({}), &ctx).unwrap_err();
        assert!(matches!(err, TransformError::InvalidInput { .. }));
        let err2 = QuotedPrintableDecode.apply(Cow::Borrowed(b"=A"), &serde_json::json!({}), &ctx).unwrap_err();
