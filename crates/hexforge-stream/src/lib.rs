...
///
/// FR-5.2 называет 64 МБ для файлового чтения в выделенных тредах; для
/// синхронного MVP-планировщика 1 МиБ — компромисс между накладными расходами
/// на вызов `apply_chunk` и гранулярностью кооперативной отмены (проверка
/// `is_cancelled()` между чанками). Константа публичная: планировщик стриминга
/// следующего поколения переопределит её на 64 МБ без изменения API.
