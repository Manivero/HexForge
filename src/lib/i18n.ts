// Minimal i18n for HexForge — FR NFR-8: architectural readiness, en/ru.
// Keys are flat dot-notation; missing key falls back to key itself.

export type Locale = "en" | "ru";

export const translations: Record<Locale, Record<string, string>> = {
  en: {
    "app.emptyGraph": "Empty graph — start with ⌘K",
    "app.nodes": "{count} node(s)",
    "app.selected": "selected: {id}",
    "app.addOperation": "Add operation",
    "app.cancel": "Cancel",
    "app.runNode": "Run node",
    "app.running": "Running…",
    "app.sourceReady": "source ready",
    "app.noSource": "no source",
    "app.snapshots": "{count} snapshot(s)",
    "app.version": "HexForge v0.1.0 — Node Graph MVP",
    "app.selectNodeForRun": "Select a node to run (⌘K → operation)",
    "app.export": "Export",
    "app.import": "Import",
    "app.exportPrompt": "Enter export path (e.g. recipe.hexforge)",
    "app.exportCyberChef": "Export CyberChef",
    "app.exportCyberChefPrompt": "Enter CyberChef export path (e.g. recipe.cyberchef.json)",
    "app.exportCyberChefWarnings": "Export completed with warnings:",
    "app.exportOutput": "Export Result To File",
    "app.exportOutputPrompt": "Enter output file path (e.g. output.bin)",
    "app.exportOutputDone": "Successfully exported {size} bytes",
  // FR-2.5 — panel visibility toggles
  "app.toggleInspector": "Toggle Inspector",
  "app.toggleInput": "Toggle Input",
  "app.toggleHistory": "Toggle History",
  "app.togglePlugins": "Toggle Plugins",
  "app.togglePreview": "Toggle Preview",
    "app.importPrompt": "Enter import path",
    "app.missingOps": "Missing operations: {ops}",
    "app.missingPlugins": "Missing plugins: {plugins}",
    "app.diffNeedTwo": "Select two snapshots to diff",
    "app.memoryWarning": "This operation requires O(n) memory ({size} MB). Continue?",
    "app.memoryWarningLarge": "This operation requires O(n) memory ({size} MB). Large inputs may cause slowdowns. Continue?",
    "app.runAnyways": "Run anyways",
    "input.placeholder": "Enter text or drop file…",
    "history.title": "History",
    "plugins.title": "Plugins",
    "plugins.reload": "Reload",
    "plugins.loading": "Loading plugins…",
    "plugins.empty": "No plugins discovered — install one below.",
    "plugins.unavailable": "Plugin backend unavailable (running outside Tauri?)",
    "plugins.installTitle": "Install plugin",
    "plugins.wasmPath": "WASM path",
    "plugins.manifestPath": "Manifest path",
    "plugins.install": "Install",
    "plugins.signatureValid": "signature valid",
    "plugins.signatureInvalid": "INVALID SIGNATURE",
    "plugins.statusUnavailable": "ARTIFACT MISSING",
    "plugins.statusIncompatible": "INCOMPATIBLE WASM",
    "plugins.requested": "requested",
    "plugins.granted": "granted",
    "plugins.grant": "Grant",
    "plugins.revoke": "Revoke",
    "plugins.sessionNote": "Installs persist in the app library and survive restarts; grants persist per install.",
  },
  ru: {
    "app.emptyGraph": "Пустой граф — начните с ⌘K",
    "app.nodes": "{count} node(s)",
    "app.selected": "selected: {id}",
    "app.addOperation": "Add operation",
    "app.cancel": "Cancel",
    "app.runNode": "Run node",
    "app.running": "Running…",
    "app.sourceReady": "source ready",
    "app.noSource": "no source",
    "app.snapshots": "{count} snapshot(s)",
    "app.version": "HexForge v0.1.0 — Node Graph MVP",
    "app.selectNodeForRun": "Выберите узел для запуска (⌘K → операция)",
    "app.export": "Экспорт",
    "app.import": "Импорт",
    "app.exportPrompt": "Введите путь для экспорта (напр. recipe.hexforge)",
    "app.exportCyberChef": "Экспорт CyberChef",
    "app.exportCyberChefPrompt": "Введите путь экспорта CyberChef (напр. recipe.cyberchef.json)",
    "app.exportCyberChefWarnings": "Экспорт завершён с предупреждениями:",
    "app.exportOutput": "Экспорт результата в файл",
    "app.exportOutputPrompt": "Введите путь для сохранения результата (напр. output.bin)",
    "app.exportOutputDone": "Успешно экспортировано {size} байт",
  // FR-2.5 — panel visibility toggles
  "app.toggleInspector": "Переключить Inspector",
  "app.toggleInput": "Переключить Input",
  "app.toggleHistory": "Переключить History",
  "app.togglePlugins": "Переключить Plugins",
  "app.togglePreview": "Переключить Preview",
    "app.importPrompt": "Введите путь для импорта",
    "app.missingOps": "Отсутствуют операции: {ops}",
    "app.missingPlugins": "Отсутствуют плагины: {plugins}",
    "app.diffNeedTwo": "Выберите два снапшота для diff",
    "app.memoryWarning": "Операция требует O(n) памяти ({size} МБ). Продолжить?",
    "app.memoryWarningLarge": "Операция требует O(n) памяти ({size} МБ). Большие файлы могут замедлить работу. Продолжить?",
    "app.runAnyways": "Запустить всё равно",
    "input.placeholder": "Введите текст или перетащите файл…",
    "history.title": "История",
    "plugins.title": "Плагины",
    "plugins.reload": "Обновить",
    "plugins.loading": "Загрузка плагинов…",
    "plugins.empty": "Плагины не обнаружены — установите ниже.",
    "plugins.unavailable": "Бэкенд плагинов недоступен (запуск вне Tauri?)",
    "plugins.installTitle": "Установка плагина",
    "plugins.wasmPath": "Путь WASM",
    "plugins.manifestPath": "Путь манифеста",
    "plugins.install": "Установить",
    "plugins.signatureValid": "подпись верна",
    "plugins.signatureInvalid": "НЕВЕРНАЯ ПОДПИСЬ",
    "plugins.statusUnavailable": "АРТЕФАКТ ОТСУТСТВУЕТ",
    "plugins.statusIncompatible": "НЕСОВМЕСТИМЫЙ WASM",
    "plugins.requested": "запрошено",
    "plugins.granted": "выдано",
    "plugins.grant": "Выдать",
    "plugins.revoke": "Отозвать",
    "plugins.sessionNote": "Установки сохраняются в библиотеке и переживают рестарты; гранты хранятся отдельно.",
  },
};

export function t(locale: Locale, key: string, params?: Record<string, string | number>): string {
  const dict = translations[locale] ?? translations.en;
  let tmpl = dict[key] ?? translations.en[key] ?? key;
  if (params) {
    for (const [k, v] of Object.entries(params)) {
      tmpl = tmpl.replaceAll(`{${k}}`, String(v));
    }
  }
  return tmpl;
}
